...
class Trainer(BaseTrainer):
    ...
    def train(self):
        ...
        reader = Reader(
            ...
            max_sapo_length=args.max_sapo_length,
            user2id=self._user2id,
            category2id=self._category2id,
            max_his_click=args.his_length,
            npratio=args.npratio,
        )
        train_dataset = reader.read_train_dataset(
            args.data_name, args.train_news_path, args.train_behaviors_path
        )
        train_dataset.set_mode(MyDataset.TRAIN_MODE)
        ...
        if args.fast_eval:
            eval_dataset = reader.read_train_dataset(
                args.data_name, args.eval_news_path, args.eval_behaviors_path
            )
        else:
            eval_dataset = reader.read_eval_dataset(
                args.data_name, args.eval_news_path, args.eval_behaviors_path
            )
        ...
        news_encoder = NewsEncoder.from_pretrained(
            args.pretrained_embedding,
            config=config,
            apply_reduce_dim=args.apply_reduce_dim,
            use_cls_embed=args.use_cls_embed,
            ...
            num_category=len(self._category2id),
        ...
        for epoch in range(args.num_train_epochs):
            ...
            accumulation_factor = (
                ...
                if steps_in_epoch > self.args.gradient_accumulation_steps
                else steps_in_epoch
            )
            ...
            for step, batch in tqdm(
                ...
                batch_loss = self._train_step(
                    batch, model, loss_calculator, accumulation_factor
                )
                ...
                if not (
                    ...
                    if global_step % args.logging_steps == 0:
                        ...
                        self._log_train(
                            scheduler, logging_loss, global_step, epoch
                        )
                        logging_loss = 0.0

                    if global_step % args.eval_steps == 0:
                        eval_start_time = time.time()
                        valid_loss, scores = self._eval(
                            model,
                            eval_dataset,
                            loss_calculator,
                        ...
                        if (
                            "loss" in self.args.evaluation_info
                            and valid_loss < best_valid_loss
                        ):
                            self._logger.info(
                                f"Best loss updates from {best_valid_loss} to {valid_loss}, "
                        ...
                        if (
                            "metrics" in self.args.evaluation_info
                            and scores["auc"] > best_auc_score
                        ):
                            self._logger.info(
                                f'Best AUC score updates from {best_auc_score} to {scores["auc"]}, '
                                f"at global step {global_step}"
                            )
                            best_auc_score = scores["auc"]
                            self._save_model(
                                model, optimizer, scheduler, flag="bestAucModel"
                            )
            ...
            eval_start_time = time.time()
            valid_loss, scores = self._eval(
                model, eval_dataset, loss_calculator, metrics=args.metrics
            )
            ...
            if (
                ...
                self._logger.info(
                    f"Best loss updates from {best_valid_loss} to {valid_loss}, at epoch {epoch}"
                )
                best_valid_loss = valid_loss
                self._save_model(
                    model, optimizer, scheduler, flag="bestLossModel"
                )
            if (
                "metrics" in self.args.evaluation_info
                and scores["auc"] > best_auc_score
                ...
                self._logger.info(
                    f'Best AUC score updates from {best_auc_score} to {scores["auc"]}, at epoch {epoch}'
                )
                best_auc_score = scores["auc"]
                self._save_model(
                    model, optimizer, scheduler, flag="bestAucModel"
            ...
            self._logger.info(
                f"Total running time of epoch: {round(epoch_end_time - epoch_start_time, ndigits=4)} (s)"
            )
            self._logger.info(
                f"Total training time of epoch: "
    ...
    def eval(self):
        ...
        dataset = reader.read_eval_dataset(
            args.data_name, args.eval_news_path, args.eval_behaviors_path
        )
    ...
    def _train_step(
        self, batch, model, loss_calculator, accumulation_factor: int
    ):
        model.train()
    ...
    def _eval(
        ...
        model.eval()
        dataset.set_mode(MyDataset.EVAL_MODE)
        ...
        with torch.no_grad():
            for batch in tqdm(
                dataloader, total=len(dataloader), desc="Evaluation phase"
            ):
                batch = utils.to_device(batch, self._device)
                logits = self._forward_step(model, batch)
                if "loss" in self.args.evaluation_info:
                    batch_loss = loss_calculator.compute_eval_loss(
                        logits, batch["label"]
                    )
        ...
        for key in keys:
            ...
            if not batch[0][key].shape:
                ...
                if key in ["his_title", "title", "his_sapo", "sapo"]:
                    ...
                    padded_batch[key] = utils.padded_stack(
                        samples, padding=self._category2id["pad"]
                    )
                else:
                    padded_batch[key] = utils.padded_stack(samples, padding=0)
    ...
    def _forward_step(model, batch):
        logits = model(
            ...
            his_title=batch["his_title"],
            his_title_mask=batch["his_title_mask"],
            his_mask=batch["his_mask"],
            sapo=batch["sapo"],
            sapo_mask=batch["sapo_mask"],
            his_sapo=batch["his_sapo"],
            his_sapo_mask=batch["his_sapo_mask"],
