...
class BaseTrainer(ABC):
    ...
    def _init_logger(self):
        ...
        if self.args.mode == "train":
            ...
            logger_utils.create_csv(
                self._eval_csv, header=["global_step"] + eval_header
            )
            self._eval_epoch = os.path.join(self._log_path, "epoch.csv")
            logger_utils.create_csv(
                self._eval_epoch,
                header=["global_step", "train_loss"] + eval_header,
            )

            # Create tensorboard log
            os.makedirs(
                os.path.join(self._path, self.args.tensorboard_path),
                exist_ok=True,
            )
            ...
            os.makedirs(self._path, exist_ok=True)
            file_handler = logging.FileHandler(
                os.path.join(self._path, "all.log")
            )
    ...
    def _log_dataset(self, train_dataset: MyDataset, valid_dataset: MyDataset):
        r"""
    ...
    def _log_train_step(self, scheduler, loss, global_step):
        ...
        self._writer.add_scalar(
            "Training loss per iteration", loss, global_step=global_step
        )
        self._writer.add_scalar(
            "Learning rate per iteration", float(lr[0]), global_step=global_step
        )
    ...
    def _log_train(self, scheduler, loss, global_step, epoch):
        ...
        self._writer.add_scalar(
            "Training loss per logging step", loss, global_step=global_step
        )

    def _log_epoch(
        self, train_loss: float, valid_loss: float, scores: dict, epoch: int
    ):
        r"""
        ...
        self._writer.add_scalar(
            "Training loss per epoch", train_loss, global_step=epoch
        )
        if "loss" in self.args.evaluation_info:
            data.append(valid_loss)
            self._logger.info(f"Validation loss: {valid_loss}")
            self._writer.add_scalar(
                "Validation loss per epoch", valid_loss, global_step=epoch
            )
        if "metrics" in self.args.evaluation_info:
            ...
            self._writer.add_scalar(
                "Validation AUC score per epoch",
                scores["auc"],
                global_step=epoch,
            )
    ...
    def _log_eval(self, global_step, loss: float = None, scores: dict = None):
        ...
        """
        self._logger.info(
            f"------  Evaluation at global step {global_step}  ------"
        )
        data = [global_step]
        if "loss" in self.args.evaluation_info:
            data.append(loss)
            ...
            self._writer.add_scalar(
                "Validation loss per step", loss, global_step=global_step
            )
        if "metrics" in self.args.evaluation_info:
            for metric, score in scores.items():
                self._logger.info(f"{metric}: {score}")
