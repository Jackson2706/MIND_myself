...
from src.dataset import MyDataset, News
...
class Reader:
    ...
    def read_train_dataset(
        self, data_name: str, news_path: str, behaviors_path: str
    ) -> MyDataset:
    ...
    def _read(self, data_name: str, news_path: str) -> Tuple[MyDataset, dict]:
        dataset = MyDataset(data_name, self._category2id)
        news_dataset = self._read_news_info(news_path, dataset)

        return dataset, news_dataset

    def _read_news_info(self, news_path: str, dataset: MyDataset):
        r"""
        ...
        with open(news_path, mode="r", encoding="utf-8", newline="") as f:
            ...
            for line in news_tsv:
                title_encoding = line[constants.TITLE]
                category_id = self._category2id.get(
                    line[constants.CATEGORY], self._category2id["unk"]
                )
                sapo_encoding = line[constants.SAPO]
                news = dataset.create_news(
                    title_encoding, sapo_encoding, category_id
                )
    ...
    def _parse_train_line(self, impression_id, line, news_dataset, dataset):
        ...
        Returns:
            None
        """
        user_id = self._user2id.get(
            line[constants.USER_ID], self._user2id["unk"]
        )
        history_clicked = [
            news_dataset[news_id] for news_id in line[constants.HISTORY].split()
        ]
        history_clicked = [news_dataset["pad"]] * (
            self._max_his_click - len(history_clicked)
        ) + history_clicked[: self._max_his_click]
        pos_news = [
            news_dataset[news_id]
        ...
        for news in pos_news:
            ...
            list_news = [news] + sample_news(
                neg_news, self._npratio, news_dataset["pad"]
            )
            impression_news = list(zip(list_news, label))
            random.shuffle(impression_news)
            list_news, label = zip(*impression_news)
            impression = dataset.create_impression(
                impression_id, user_id, list_news, label
            )
    ...
    def _parse_eval_line(self, impression_id, line, news_dataset, dataset):
        ...
        for behavior in line[constants.BEHAVIOR].split():
            ...
            impression = dataset.create_impression(
                impression_id, user_id, [news_dataset[news_id]], [int(label)]
            )
            dataset.add_sample(user_id, history_clicked, impression)
