...
class BaseEvaluator(ABC):
    ...
    def compute_scores(
        self, metrics: List[str], save_result: bool, path: str = None
    ):
        self._convert_pred()
        ...
        for metric in metrics:
            if metric == "auc":
                ...
                list_score = [
                    roc_auc_score(y_true=target, y_score=prob_prediction)
                    for target, prob_prediction in zip(
                        self.targets, self.prob_predictions
                    )
                ]
                ...
                list_score = [
                    compute_mrr_score(
                        np.array(target), np.array(prob_prediction)
                    )
                    for target, prob_prediction in zip(
                        self.targets, self.prob_predictions
                    )
                ...
                list_score = [
                    compute_ndcg_score(
                        ...
                        y_score=np.array(prob_prediction),
                        k=k,
                    )
                    for target, prob_prediction in zip(
                        self.targets, self.prob_predictions
...
class SlowEvaluator(BaseEvaluator):
    ...
    def _convert_targets(self):
        ...
        for sample in self.dataset.samples:
            ...
            group_labels[impression_id] = (
                group_labels.get(impression_id, []) + sample.impression.label
            )
    ...
    def _convert_pred(self):
        group_predictions = {}
        for prob_prediction, impression_id in zip(
            self.prob_predictions, self.impression_ids
        ):
            group_predictions[impression_id] = (
                group_predictions.get(impression_id, []) + prob_prediction
            )
