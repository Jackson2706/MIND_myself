...
class NewsEncoder(ABC, RobertaPreTrainedModel):
    def __init__(
        ...
        use_sapo: bool,
        use_category: bool,
        category_pad_token_id: int,
        query_dim: int,
        dropout: float,
        word_embed_dim: Union[int, None] = None,
        category_embed_dim: Union[int, None] = None,
        category_embed: Union[Tensor, None] = None,
        num_category: Union[int, None] = None,
        num_heads: Union[int, None] = None,
        attn_dropout: Union[float, None] = None,
        ...
        if self.apply_reduce_dim:
            ...
            self.reduce_dim = nn.Linear(
                in_features=config.hidden_size, out_features=word_embed_dim
            )
        ...
        if not self.use_cls_embed:
            ...
            self.title_additive_attn = AdditiveAttention(
                query_dim=query_dim, embed_dim=self._embed_dim
            )

            # Sapo encoder
            if self.use_sapo:
                self.sapo_self_attn = nn.MultiheadAttention(
                    embed_dim=self._embed_dim,
                    num_heads=num_heads,
                    dropout=attn_dropout,
                    batch_first=True,
                ...
                self.sapo_additive_attn = AdditiveAttention(
                    query_dim=query_dim, embed_dim=self._embed_dim
                )
                total_size += self._embed_dim
        ...
        if self.use_sapo or self.use_category:
            ...
            if self.combine_type == "linear":
                self.linear_combine = nn.Linear(
                    in_features=total_size, out_features=self._embed_dim
                )
            elif self.combine_type == "attn_pooling":
                if self.use_category:
                    self.category_dense_layer = nn.Sequential(
                        nn.Linear(
                            in_features=category_embed_dim,
                            out_features=self._embed_dim,
                        ),
                        nn.ReLU(),
                    )
                self.attn_pooling = AdditiveAttention(
                    query_dim=query_dim, embed_dim=self._embed_dim
                )
    ...
    def forward(
        self,
        title_encoding: Tensor,
        title_attn_mask: Tensor,
        sapo_encoding: Union[Tensor, None] = None,
        ...
        if self.use_cls_embed:
            ...
            title_word_context, _ = self.title_self_attn(
                ...
                value=title_word_embed,
                key_padding_mask=~title_attn_mask,
            )
            title_word_context = self.title_dropout(title_word_context)
            title_repr = self.title_additive_attn(
                embeddings=title_word_context, mask=title_attn_mask
            )
        ...
        if self.use_sapo:
            ...
            if self.use_cls_embed:
                ...
                sapo_word_context, _ = self.sapo_self_attn(
                    ...
                    key=sapo_word_embed,
                    value=sapo_word_embed,
                    key_padding_mask=~sapo_attn_mask,
                )
                sapo_word_context = self.sapo_dropout(sapo_word_context)
                sapo_repr = self.sapo_additive_attn(
                    embeddings=sapo_word_context, mask=sapo_attn_mask
                )
        ...
        if self.use_category or self.use_sapo:
            if self.combine_type == "linear":
                ...
                mask = torch.ones(
                    (news_info.shape[0], news_info.shape[1]),
                    dtype=torch.bool,
                    device=news_info.device,
                )
