...
class NRMS(nn.Module):
    ...
    def forward(
        ...
        title_mask: Tensor,
        his_title: Tensor,
        his_title_mask: Tensor,
        his_mask: Tensor,
        sapo: Union[Tensor, None] = None,
        sapo_mask: Union[Tensor, None] = None,
        his_sapo: Union[Tensor, None] = None,
        his_sapo_mask: Union[Tensor, None] = None,
        category: Union[Tensor, None] = None,
        ...
        candidate_news_repr = self.news_encoder(
            title_encoding=title,
            title_attn_mask=title_mask,
            sapo_encoding=sapo,
            sapo_attn_mask=sapo_mask,
            category_encoding=category,
        )
        candidate_news_repr = candidate_news_repr.view(
            batch_size, num_candidates, -1
        )

        # Representation of the users
        user_repr = self.user_encoder(
            title_encoding=his_title,
            title_attn_mask=his_title_mask,
            history_mask=his_mask,
            sapo_encoding=his_sapo,
            sapo_attn_mask=his_sapo_mask,
            category_encoding=his_category,
        )

        # Click predictor
        logits = torch.bmm(
            candidate_news_repr, user_repr.unsqueeze(dim=2)
        ).squeeze(dim=2)
