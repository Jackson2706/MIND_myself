...
class UserEncoder(nn.Module):
    def __init__(
        ...
        self.multi_head_self_attn = nn.MultiheadAttention(
            embed_dim=news_encoder.embed_dim,
            num_heads=num_heads,
            dropout=attn_dropout,
            batch_first=True,
        )
        self.additive_attn = AdditiveAttention(
            query_dim=query_dim, embed_dim=news_encoder.embed_dim
        )
    ...
    def forward(
        ...
        title_attn_mask = title_attn_mask.view(
            batch_size * num_clicked_news, -1
        )
