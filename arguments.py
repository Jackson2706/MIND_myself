...
def add_eval_arguments(parser: argparse.ArgumentParser):
    _add_common_arguments(parser)
    parser.add_argument(
        "--saved_model_path", type=str, help="Path to the trained model"
    )
    parser.add_argument(
        "--data_name", type=str, help="Name of the eval dataset"
    )
    parser.add_argument(
        "--eval_behaviors_path",
        type=str,
    ...
    parser.add_argument(
        "--eval_news_path",
        type=str,
        help="Path to the news.tsv file for the evaluation phase",
    )
    parser.add_argument(
        "--fast_eval",
        action="store_true",
        help="Is there a fast evaluation for the eval dataset?",
    )
    parser.add_argument(
        "--eval_batch_size",
        type=int,
        help="How many samples per batch to load in the test phase",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="If True, the data loader will copy "
...
def _add_common_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--user2id_path", type=str, help="Path to the user dictionary"
    )
    parser.add_argument(
        "--category2id_path", type=str, help="Path to the category dictionary"
    )
    parser.add_argument(
        "--category_embed_path",
        type=str,
    ...
    parser.add_argument(
        "--max_title_length",
        type=int,
        help="The maximum length of a title encodes",
    )
    parser.add_argument(
        "--max_sapo_length",
        type=int,
        help="The maximum length of a sapo encodes",
    )
    parser.add_argument(
        "--his_length", type=int, help="Max number of user click history"
    )
    parser.add_argument("--seed", type=int, help="Seed value")
    parser.add_argument(
        "--save_eval_result",
        action="store_true",
        help="Whether or not to save the evaluation result",
    )
    parser.add_argument(
        "--metrics",
        type=str,
        nargs="+",
        help="List of metrics used for evaluation",
    )
    parser.add_argument(
        "--evaluation_info",
        type=str,
...
def _add_train_data_args(parser: argparse.ArgumentParser):
    parser.add_argument(
        "--data_name", type=str, default=None, help="Name of the train dataset"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--train_news_path",
        type=str,
        help="Path to the news.tsv file for the training phase",
    )
    parser.add_argument(
        "--eval_behaviors_path",
        type=str,
...
def _add_model_args(parser: argparse.ArgumentParser):
    parser.add_argument(
        ...
        type=str,
        help="Path to the pre-trained Roberta model",
    )
    parser.add_argument(
        "--apply_reduce_dim",
        action="store_true",
    ...
    parser.add_argument(
        "--use_sapo",
        action="store_true",
        help="Whether to use sapo embedding or not",
    )
    parser.add_argument(
        "--use_category",
        action="store_true",
        help="Whether to use category embedding or not",
    )
    parser.add_argument(
        "--query_dim",
        type=int,
        help="The size of query vector in the additive attention network",
    )
    parser.add_argument(
        "--word_embed_dim",
        type=int,
        help="The size of each word embedding vector if apply_reduce_dim",
    )
    parser.add_argument(
        "--category_embed_dim",
        type=int,
        help="The size of each category embedding vector",
    )
    parser.add_argument(
        "--num_heads",
        type=int,
        help="Number of parallel attention heads in Multi-head attention",
    )
    parser.add_argument(
        "--combine_type", type=str, choices=["linear", "attn_pooling"]
    )
    parser.add_argument(
        "--attn_dropout",
        type=float,
        help="Dropout probability in the multi-head attention network",
    )
...
def _add_train_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--train_batch_size",
        type=int,
        help="How many samples per batch to load in the training phase",
    )
    parser.add_argument(
        "--eval_batch_size",
        type=int,
    ...
    parser.add_argument(
        "--max_grad_norm", type=float, help="Max norms of the gradients"
    )
    parser.add_argument(
        "--weight_decay", type=float, help="Decoupled weight decay to apply"
    )
    parser.add_argument(
        "--logging_steps",
        type=int,
